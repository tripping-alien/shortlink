...
def init_firebase():
    global db, APP_INSTANCE, DB_INITIALIZED
    if db:
        return db
...
def create_link_in_db(long_url: str, ttl: str, custom_code: Optional[str] = None):
    code = custom_code.lower() if custom_code else generate_unique_short_code()
...
<style>
:root {
--primary-color:#4f46e5;--secondary-color:#6366f1;--accent-color:#facc15;
--bg-color:#f3f4f6;--text-color:#111827;
}
...
async def api_create_link(payload: Dict[str, Any]):
    ...
    try:
        ...
        return {"short_url": preview_url, "deletion_token": link["deletion_token"]}
