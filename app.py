...
# --- UPDATED: /preview/{short_code} route ---
...
def update_clicks_in_transaction(transaction, doc_ref) -> str:
    ...
    expires_at = link.get("expires_at")
    if expires_at and expires_at < datetime.now(timezone.utc):
        raise HTTPException(status_code=4Setting up caching is the final and most powerful performance optimization we can make.

This is a **purely backend change**. You will not need to modify any of your `.html`, `.css`, or `requirements.txt` files.

The logic will be as follows:
1.  **When a link is created (`create_link_in_db`):** We will add new, empty fields to the Firestore document: `meta_fetched: False`, `meta_title: None`, `meta_description: None`, etc.
2.  **When a preview page is visited (`/preview/{short_code}`):**
    * **Fast Path (Cache Hit):** The server will check if `meta_fetched` is `True`. If it is, it will *immediately* serve the title, description, and image from the database. This is nearly instantaneous.
    * **Slow Path (Cache Miss):** If `meta_fetched` is `False`, the server will perform the slow `await fetch_metadata(...)` call. It will then *save* the results to the Firestore document and set `meta_fetched` to `True`. The first user has a slow load, but every user after them gets the fast, cached version.

Here is the complete, updated `main.py` file with the caching logic implemented.

---

### `main.py` (Complete and Updated)
...
# ---------------- CONFIG ----------------
BASE_URL = os.environ.get("BASE_URL", "[https://shortlinks.art](https://shortlinks.art)")
SHORT_CODE_LENGTH = 6
MAX_ID_RETRIES = 10
...
ADSENSE_CLIENT_ID = "pub-6170587092427912"
ADSENSE_SCRIPT = f"""
    <script async src="[https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js?client=](https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js?client=){ADSENSE_CLIENT_ID}"
     crossorigin="anonymous"></script>
"""
...
async def redirect_link(short_code: str):
    ...
    try:
        ...
        # Check for expiration on fallback
        expires_at = link.get("expires_at")
        if expires_at and expires_at < datetime.now(timezone.utc):
            raise HTTPException(status_code=410, detail="Link expired")
